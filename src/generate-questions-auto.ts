...
async function checkCurrentQuestionCounts(): Promise<Record<string, Record<string, number>>> {
  ...
  for (const topic of TOPICS) {
    ...
    DIFFICULTY_LEVELS.forEach(diff => {
      difficultyTotals[diff as keyof typeof difficultyTotals] += counts[diff];
    });
  }
  
  console.log('\nTotais por nível:');
  DIFFICULTY_LEVELS.forEach(diff => {
    const total = difficultyTotals[diff as keyof typeof difficultyTotals];
...
async function getQuestionCountForTopicAndDifficulty(topic: string, difficulty: string): Promise<number> {
